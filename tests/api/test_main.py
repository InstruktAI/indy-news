...
def mock_csv_data():
    ...
    return """Name,Website,Youtube,About,TrustFactors,Topics,Wikipedia,X,Substack
Al Jazeera,https://aljazeera.com,@aljazeeraenglish,International news,Independent,Politics,https://wiki,@ajenglish,n/a
Democracy Now,https://democracynow.org,@democracynow,Independent news,Grassroots,Politics,https://wiki,@democracynow,democracynow"""


@pytest.fixture
def mock_mbfc_data():
    """Mock MediaBiasFactCheck data"""
    return [
        {
            "name": "Al Jazeera",
            "bias": "Left-Center",
...
@pytest.fixture
def mock_allsides_data():
    """Mock AllSides data"""
    return [
        {"name": "Al Jazeera", "bias": "Left", "url": "https://aljazeera.com"},
        {"name": "Democracy Now", "bias": "Left", "url": "https://democracynow.org"},
    ]
...
class TestDataEndpoints:
    ...
    def test_sources_endpoint_returns_minimal_fields(
        self, client: TestClient, api_key: str, mock_csv_data: str, mock_mbfc_data: List
    ):
        """Test /sources returns only Name, About, Topics"""
        with patch("builtins.open", mock_open(read_data=mock_csv_data)):
            with patch("json.load", return_value=mock_mbfc_data):
    ...
    def test_source_media_filters_na_values(
        ...
        with patch("builtins.open", mock_open(read_data=mock_csv_data)):
            with patch("json.load", return_value=mock_mbfc_data):
                with patch("os.path.exists", return_value=False):
                    response = client.get(
                        f"/source-media?sources=Al Jazeera&apikey={api_key}"
                    )
                    assert response.status_code == 200
                    data = response.json()
                    assert len(data) == 1
    ...
    def test_allsides_search_case_insensitive(
        self, client: TestClient, api_key: str, mock_allsides_data: List
    ):
        """Test /allsides search is case-insensitive"""
        with patch("builtins.open", mock_open()):
            with patch("json.load", return_value=mock_allsides_data):
                response = client.get(f"/allsides?name=jazeera&apikey={api_key}")
                assert response.status_code == 200
                data = response.json()
                assert len(data) == 1
                assert "Jazeera" in data[0]["name"]

    def test_allsides_pagination(
        self, client: TestClient, api_key: str, mock_allsides_data: List
    ):
        """Test /allsides pagination parameters"""
        with patch("builtins.open", mock_open()):
            with patch("json.load", return_value=mock_allsides_data):
                # Test offset
                response = client.get(
                    f"/allsides?name=e&limit=1&offset=1&apikey={api_key}"
                )
                assert response.status_code == 200
                data = response.json()
                assert len(data) <= 1

    def test_mediabiasfactcheck_credibility_filter(
        self, client: TestClient, api_key: str
    ):
        """Test /mediabiasfactcheck filters by credibility/factual rating"""
        low_credibility_data = [
            {
                "name": "Bad Source",
                "bias": "Extreme Right",
                "factual": "Low",
        ...
        with patch("builtins.open", mock_open()):
            with patch("json.load", return_value=low_credibility_data):
                response = client.get(
                    f"/mediabiasfactcheck?name=Bad&apikey={api_key}"
                )
                assert response.status_code == 200
                # Low credibility AND low factual should be filtered out
                assert len(response.json()) == 0
...
class TestContentEndpointValidation:
    ...
    def test_x_requires_query_or_users(self, client: TestClient, api_key: str):
    ...
    def test_news_requires_channels_or_users(self, client: TestClient, api_key: str):
...
class TestWebhookEndpoint:
    ...
    def test_webhook_accepts_valid_cookies(self, client: TestClient, api_key: str):
        """Test webhook successfully saves valid cookies"""
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("os.getenv", return_value=tmpdir):
                payload = {
                    "success": True,
    ...
    def test_webhook_rejects_failed_refresh(self, client: TestClient, api_key: str):
...
class TestHealthEndpoint:
    ...
    def test_privacy_endpoint(self, client: TestClient):
